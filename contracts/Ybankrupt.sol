...
interface IYbankruptV1Helper {
    function getQuoteLimit(uint gasUsed) external view returns (uint);
}

contract YbankruptV1 is ReentrancyGuard {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    /// @notice Ybankrupt Helper to set max prices for the ecosystem
    IYbankruptV1Helper public KPRH;

    /// @notice EIP-20 token name for this token
    string public constant name = "YbankruptV1";

    /// @notice EIP-20 token symbol for this token
    string public constant symbol = "BNKRPT";
    ...
    function applyCreditToJob(address provider, address liquidity, address job) external {
        ...
        uint _liquidity = YbankruptV1Library.getReserve(liquidity, address(this));
    ...
    /**
     * @notice Allows governance to change the YbankruptHelper for max spend
     * @param _kprh new helper address to set
     */
    function setYbankruptHelper(IYbankruptV1Helper _kprh) external {
        require(msg.sender == governance, "setYbankruptHelper: !gov");
        KPRH = _kprh;
    }
